...
// Model for UserV entity
    models.UserV = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/infraBike/webapi/uk.ac.dundee.computing.infrabike.dto.userv/",
        idAttribute: 'idUser',
        defaults: {
            password: "",
            idRole: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('idUser');
            return result;
        },
        isNew: function () {
    ...
    // Collection class for UserV entities
    models.UserVCollection = Backbone.Collection.extend({
        model: models.UserV,
        url: "http://localhost:8080/infraBike/webapi/uk.ac.dundee.computing.infrabike.dto.userv/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.UserVCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-userv-list-itemis template identifier for item
                        templateName: '#tpl-userv-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.userv = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.userv,
                    // tpl-userv-details is template identifier for chosen model element
                    templateName: '#tpl-userv-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.UserV();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-userv-details is a template identifier for chosen model element
                templateName: '#tpl-userv-details',
